...
public class PlayerRunner : MonoBehaviour {
	...
	void Update () {
		...
		if (Input.GetAxis("Vertical") == 1)
	    {
			for (int foot = 0; foot < 2; foot++)
			{
				...
				if (step[foot] <= 0.75F && foothold[foot] != Vector3.zero)
				{
					...
					{
						//Debug.DrawLine(transform.position, foothold[foot], Color.blue, 300, true);
						velocity += (PlayerHead.transform.TransformDirection(Vector3.forward + Vector3.up*20)) * 2;
					} else {
		...
		oldPosition = transform.position;
		controller.Move(velocity * top.deltaTime);	// Using char controller collisions
	}
}
